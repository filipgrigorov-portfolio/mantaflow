...
GRID_SIZE = 64
INPUT_CHANNELS = 7
TOTAL_SIMULATION_TIME = 16 # length of sequences
...
def sample(ddpm, dt_prev, vt_prev, bc_init, tau, output_channels, n, device=None, grid_size=GRID_SIZE):
    ...
    for _, t in enumerate(list(range(ddpm.diffusion_steps))[::-1]):
        ...
        eta_theta = ddpm.backward(x_stacked, time_tensor, tau.reshape(n, -1))
        ...
        if t > 0:
            ...
            z = torch.randn(n, output_channels, grid_size, grid_size).to(device)
