...
def train(display=True, continue_from_checkpoint=False, show_forward_process=False):
    ...
    for epoch in tqdm(range(EPOCHS), desc=f"Training progress", colour="#00ff00"):
        ...
        if display and epoch % AT_EVERY == AT_EVERY -1 :
            ...
            show_compound_images(sample(ddpm, dt_prev, vt_prev, bc0, tau, 
                output_channels=OUTPUT_CHANNELS, n=BATCH_SIZE, device=DEVICE), f"Images generated at epoch {epoch + 1}")
...
if __name__ == "__main__":
    ...
    if args.show_data:
        ...
        print('Training mode')
        train(continue_from_checkpoint=args.from_checkpoint, show_forward_process=False)
    elif args.infer:
